...
import { TopGame } from '@/types/games';
...
import { Check, CheckCircle2, Gamepad2, Plus, Trash2 } from 'lucide-react';
    ...
    <DropdownMenu open={open} onOpenChange={setOpen} modal={false}>
      <div className="group relative w-full flex flex-col" style={{ width: 'var(--item-width, 100%)', maxWidth: 'var(--item-width, 100%)', minWidth: 0, overflow: 'visible' }}>
        ...
        >
          ...
          {item.rating && (
            <div className="absolute right-1.5 top-1.5 rounded-md bg-black/60 px-2 py-1 text-xs font-bold text-white backdrop-blur-md flex items-center gap-1 z-10">
              <span className="text-yellow-400 text-sm">★</span> {(item.rating * 2).toFixed(1)}
            </div>
          )}

          {/* Status Badge - Below Category */}
          {alreadyInList && !isPlayed && !isPlaying && (
          ...
          {onDelete && (alreadyInList || isPlayed || isPlaying) && (
        ...
        >
          {!isPlayed && onMarkPlayed && (
            <DropdownMenuItem onClick={onMarkPlayed} disabled={isMarkingPlayed} className="rounded-lg focus:bg-accent/50 focus:text-accent-foreground cursor-pointer">
              <CheckCircle2 className="mr-2 h-4 w-4" />
              {isMarkingPlayed ? 'Marking...' : 'Mark Played'}
            </DropdownMenuItem>
          )}
          {!isPlaying && onMarkPlaying && (
            <DropdownMenuItem onClick={onMarkPlaying} disabled={isMarkingPlaying} className="rounded-lg focus:bg-accent/50 focus:text-accent-foreground cursor-pointer">
              <Gamepad2 className="mr-2 h-4 w-4" />
              {isMarkingPlaying ? 'Marking...' : 'Mark Playing'}
            </DropdownMenuItem>
          )}
        </DropdownMenuContent>
      </div>
