/**
 * TikTok Likes Import Script
 *
 * Reads TikTok likes from Excel file and imports them into the database.
 * Fetches oEmbed metadata for thumbnails and titles.
 *
 * Usage: npx tsx src/scripts/import-tiktok.ts
 */

import * as XLSX from 'xlsx';
import { PrismaClient } from '@prisma/client';
import { extractVideoId, fetchTikTokOEmbed, sleep } from '../lib/tiktok-utils';
import * as path from 'path';

const prisma = new PrismaClient();

const BATCH_SIZE = 100;
const BATCH_DELAY_MS = 2000;

interface ExcelRow {
  Date: string | number;
  Link: string;
}

interface VideoData {
  videoId: string;
  url: string;
  likedAt: Date;
}

/**
 * Parse Excel serial date to JavaScript Date
 */
function parseExcelDate(value: string | number): Date {
  if (typeof value === 'number') {
    // Excel serial date (days since 1900-01-01)
    const excelEpoch = new Date(1900, 0, 1);
    const days = value - 2; // Excel has a bug where it thinks 1900 was a leap year
    return new Date(excelEpoch.getTime() + days * 24 * 60 * 60 * 1000);
  }
  // Try parsing as string date
  return new Date(value);
}

/**
 * Read and parse the Excel file
 */
function readExcelFile(filePath: string): VideoData[] {
  console.log(`Reading Excel file: ${filePath}`);

  const workbook = XLSX.readFile(filePath);
  const sheetName = workbook.SheetNames[0];
  const worksheet = workbook.Sheets[sheetName];

  // Convert to JSON with header row
  const rawData = XLSX.utils.sheet_to_json<ExcelRow>(worksheet, { header: ['Date', 'Link'] });

  // Skip header row if it exists
  const data = rawData.filter(row =>
    row.Link &&
    typeof row.Link === 'string' &&
    row.Link.includes('tiktok')
  );

  console.log(`Found ${data.length} TikTok links in Excel file`);

  const videos: VideoData[] = [];
  let skipped = 0;

  for (const row of data) {
    const videoId = extractVideoId(row.Link);
    if (!videoId) {
      skipped++;
      continue;
    }

    const likedAt = parseExcelDate(row.Date);
    if (isNaN(likedAt.getTime())) {
      console.warn(`Invalid date for video ${videoId}: ${row.Date}`);
      continue;
    }

    videos.push({
      videoId,
      url: row.Link,
      likedAt,
    });
  }

  if (skipped > 0) {
    console.log(`Skipped ${skipped} rows without valid video IDs`);
  }

  return videos;
}

/**
 * Convert tiktokv.com URL to standard tiktok.com URL for oEmbed
 */
function convertToTikTokUrl(url: string): string {
  // Replace tiktokv.com with tiktok.com and adjust the path format
  if (url.includes('tiktokv.com')) {
    // Convert: https://www.tiktokv.com/share/video/{ID}/ to https://www.tiktok.com/video/{ID}
    return url
      .replace('tiktokv.com', 'tiktok.com')
      .replace('/share/video/', '/@placeholder/video/');
  }
  return url;
}

/**
 * Process a batch of videos
 */
async function processBatch(
  videos: VideoData[],
  batchNumber: number,
  totalBatches: number
): Promise<{ successful: number; failed: number }> {
  let successful = 0;
  let failed = 0;

  for (const video of videos) {
    // Fetch oEmbed data (convert URL for API compatibility)
    const oembedUrl = convertToTikTokUrl(video.url);
    const oembedData = await fetchTikTokOEmbed(oembedUrl);

    try {
      await prisma.tikTokVideo.upsert({
        where: { videoId: video.videoId },
        update: {
          url: video.url,
          title: oembedData?.title || null,
          thumbnail: oembedData?.thumbnail || null,
          authorName: oembedData?.authorName || null,
          likedAt: video.likedAt,
        },
        create: {
          videoId: video.videoId,
          url: video.url,
          title: oembedData?.title || null,
          thumbnail: oembedData?.thumbnail || null,
          authorName: oembedData?.authorName || null,
          likedAt: video.likedAt,
        },
      });

      if (oembedData) {
        successful++;
      } else {
        failed++;
      }
    } catch (error) {
      console.error(`Failed to save video ${video.videoId}:`, error);
      failed++;
    }
  }

  console.log(`Batch ${batchNumber}/${totalBatches} complete - oEmbed success: ${successful}, failed: ${failed}`);

  return { successful, failed };
}

/**
 * Main import function
 */
async function main() {
  const startTime = Date.now();
  console.log('='.repeat(60));
  console.log('TikTok Likes Import Script');
  console.log('='.repeat(60));

  // Read Excel file
  const excelPath = path.join(process.cwd(), 'public', 'TikTok_Likes.xlsx');
  const videos = readExcelFile(excelPath);

  if (videos.length === 0) {
    console.log('No videos to import');
    return;
  }

  console.log(`\nStarting import of ${videos.length} videos...`);
  console.log(`Batch size: ${BATCH_SIZE}, Delay between batches: ${BATCH_DELAY_MS}ms`);
  console.log('-'.repeat(60));

  // Process in batches
  const totalBatches = Math.ceil(videos.length / BATCH_SIZE);
  let totalSuccessful = 0;
  let totalFailed = 0;

  for (let i = 0; i < videos.length; i += BATCH_SIZE) {
    const batchNumber = Math.floor(i / BATCH_SIZE) + 1;
    const batch = videos.slice(i, i + BATCH_SIZE);

    const { successful, failed } = await processBatch(batch, batchNumber, totalBatches);
    totalSuccessful += successful;
    totalFailed += failed;

    // Delay between batches (except for last batch)
    if (i + BATCH_SIZE < videos.length) {
      await sleep(BATCH_DELAY_MS);
    }
  }

  const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);

  console.log('-'.repeat(60));
  console.log('\nImport Summary:');
  console.log(`  Total videos processed: ${videos.length}`);
  console.log(`  oEmbed fetches successful: ${totalSuccessful}`);
  console.log(`  oEmbed fetches failed: ${totalFailed}`);
  console.log(`  Time elapsed: ${elapsed}s`);
  console.log('='.repeat(60));

  // Verify database count
  const dbCount = await prisma.tikTokVideo.count();
  console.log(`\nTotal videos in database: ${dbCount}`);
}

main()
  .catch(console.error)
  .finally(() => prisma.$disconnect());
